...
class FPMC():
    ...
    def evaluation(self, data_list):
        ...
        for (u, i, b_tm1) in data_list:
            ...
            # 计算命中率
            hit_rate = 1 if scores.argmax() == i else 0
        ...
        hit_rate_avg = total_hit_rate / total_samples

        return precision_avg, recall_avg, hit_rate_avg, mrr_avg
    ...
    def learnSBPR_FPMC(self, tr_data, te_data=None, n_epoch=5, neg_batch_size=5, eval_per_epoch=True,
        ...
        global precision_out, recall_out, hr_out, mrr_out
        for epoch in range(n_epoch):
            self.learn_epoch(tr_data, neg_batch_size)

            if eval_per_epoch:
                precision_in, recall_in, hr_in, mrr_in = self.evaluation(tr_data)
                if te_data is not None:
                    precision_out, recall_out, hr_out, mrr_out = self.evaluation(tr_data)
                    print(
                        'Epoch %d: In sample - Precision=%.4f, Recall=%.4f, HR=%.4f, MRR=%.4f; Out sample - '
                        'Precision=%.4f, Recall=%.4f, HR=%.4f, MRR=%.4f' %
                        (epoch, precision_in, recall_in, hr_in, mrr_in, precision_out, recall_out, hr_out, mrr_out))
                else:
                    print('Epoch %d: In sample - Precision=%.4f, Recall=%.4f, HR=%.4f, MRR=%.4f' %
                          (epoch, precision_in, recall_in, hr_in, mrr_in))
            else:
        ...
        if not eval_per_epoch:
            precision_in, recall_in, hr_in, mrr_in = self.evaluation(tr_data)
            if te_data is not None:
                precision_out, recall_out, hr_out, mrr_out = self.evaluation(te_data)
                print(
                    'Precision (In/Out): %.4f/%.4f, Recall (In/Out): %.4f/%.4f, HR (In/Out): %.4f/%.4f, '
                    'MRR (In/Out): %.4f/%.4f' %
                    (precision_in, precision_out, recall_in, recall_out, hr_in, hr_out, mrr_in, mrr_out))
            else:
                print('Precision: %.4f, Recall: %.4f, HR: %.4f, MRR: %.4f' % (precision_in, recall_in, hr_in, mrr_in))

        if te_data is not None:
            return precision_out, recall_out, hr_out, mrr_out
        else:
            return None
